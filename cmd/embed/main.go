...
func parse(fs *flag.FlagSet, args []string) (*option, error) {
	...
	fs.StringVar(&o.nats, "c", "/etc/nats.conf", "nats config")
...
func run(ctx context.Context, opts *option) error {
	ns, err := newServerNATS(opts.nats)
	if err != nil {
		return err
	}
	ns.Start()
...
func newServerNATS(configFile string) (*server.Server, error) {
	o := &server.Options{
		// ServerName: "",
	}
	err := o.ProcessConfigFile(configFile)
	if err != nil {
		return nil, fmt.Errorf("process config file: %w", err)
	}
...
func waitForServer(ns *server.Server) {
	b := backoff.NewExponentialBackOff()

	for {
		d := b.NextBackOff()
		ready := ns.ReadyForConnections(d)
		if ready {
			break
		}
